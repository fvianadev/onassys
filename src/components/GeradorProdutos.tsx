  ...
  const handleImprimir = () => {
    const printContent = `
      ...
      <body>
        ...
        <table>
          <tr><th>Descrição</th><th class="right">QTD / ${uniMedidaSigla}</th><th class="right">Vl Total</th></tr>
          ${dadosTabela.filter(d => d.materialId).map(d => `
            <tr>
              <td>${d.materialNome}</td>
          ...
          `).join('')}
          <tr class="total"><td>VALOR DE CUSTO UNITÁRIOS</td><td></td><td class="right">R$ ${totalCustoProd.toFixed(2).replace('.', ',')}</td></tr>
        </table>
      </body>
      </html>
    `;
          ...
          <div className="bg-white dark:bg-[#120c06] rounded-xl border border-amber-100 dark:border-[#2d1e0d] p-4 space-y-3">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-10 gap-2 items-center pt-2 border-t border-amber-100 dark:border-[#2d1e0d]">
                <div className="col-span-4 text-[10px] font-bold text-amber-900 dark:text-amber-200">VALOR DE CUSTO UNITÁRIOS</div>
